...
public class FailedMapActivity<callStateListener, firebaseHelper> extends AppCompatActivity implements OnMapReadyCallback, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, com.google.android.gms.location.LocationListener, NavigationView.OnNavigationItemSelectedListener {
...
//    static Double costTotalLast1 = 0.00;
...
//        startForegroundService();
        ...
        setContentView(R.layout.pixel_2_all);
...
//        SharedPreferences mPrefsvehiclecharge1 = getSharedPreferences("driverPackageDetails", 0);
//        str_package_initial_charge = mPrefsvehiclecharge1.getString("initialcost", "0.00");
//        str_package_initial_distance = mPrefsvehiclecharge1.getString("initialdistance", "0.00");
//        str_package_initialhours = mPrefsvehiclecharge1.getString("initialhours", "0.00");
//        str_package_nexthourcost = mPrefsvehiclecharge1.getString("nexthourcost", "0.00");
//        str_package_nexthours = mPrefsvehiclecharge1.getString("nexthours", "0.00");
//        str_package_waitingcost = mPrefsvehiclecharge1.getString("waitingcost", "0.00");
//        str_package_cost_nextdistance = mPrefsvehiclecharge1.getString("nextcostfordistance", "0.00");
...
//        });
//
//        unFinished_Database.addValueEventListener(new ValueEventListener() {
//            @Override
//            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
//                if (dataSnapshot.exists()&& maxidstatus==0){
//                    maxid = dataSnapshot.getChildrenCount();
...
//                    FinanceBalance_Model fin = snapshot.getValue(FinanceBalance_Model.class);
        ...
        mEnd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                final AlertDialog.Builder builder = new AlertDialog.Builder(FailedMapActivity.this,R.style.MaterialThemeDialog);
                final EditText input = new EditText(FailedMapActivity.this);
                builder.setView(input);
                input.setVisibility(View.INVISIBLE);
                input.setHint("Enter voucher code here");

                final TextView myMsg = new TextView(FailedMapActivity.this);
                ...
                final TextView confirm_title = new TextView(FailedMapActivity.this);
...
//                builder.setTitle(myMsg+"Choose Payment Method");
                ...
                builder.setSingleChoiceItems(listItems, checkedItem, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        Toast.makeText(FailedMapActivity.this, "Position: " + which + " Value: " + listItems[which], Toast.LENGTH_LONG).show();
                ...
                builder.setPositiveButton("Done", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog2, int which) {
                        ...
                        final AlertDialog.Builder alert = new AlertDialog.Builder(FailedMapActivity.this,R.style.MaterialThemeDialog);
                        alert
...
//                                            WebView1.loadUrl("file:///android_asset/sample.html");
                                            ...
                                            if(to.isEmpty()){
                                                Toast.makeText(FailedMapActivity.this, "You must enter a recipient email", Toast.LENGTH_LONG).show();
                                            }else if(subject.isEmpty()){
                                                Toast.makeText(FailedMapActivity.this, "You must enter a Subject", Toast.LENGTH_LONG).show();
                                            }else if(message.isEmpty()){
                                                Toast.makeText(FailedMapActivity.this, "You must enter a message", Toast.LENGTH_LONG).show();
                                            }else {
...
//                                        Toast.makeText(DriverMapActivity.this, "Inserted", Toast.LENGTH_SHORT).show();
                                        ...
                                        Intent intent = new Intent(FailedMapActivity.this, MainActivity.class);
...
// updating  current hiring driver data
        ...
        track_Database.child(tripid).child("waitingtime").setValue(mTimer2.getText().toString().trim());
...
//        track_Database.child(tripid).child("tripid").setValue(tripid);
...
//        rDatabase.child(String.valueOf(str)).child("Location").child("Driver ID").setValue(str);
//        rDatabase.child(String.valueOf(str)).child("Location").child("lat1").setValue(lat1);
//        rDatabase.child(String.valueOf(str)).child("Location").child("lon1").setValue(lon1);
//        rDatabase.child(String.valueOf(str)).child("Location").child("lat2").setValue(lat2);
//        rDatabase.child(String.valueOf(str)).child("Location").child("lon2").setValue(lon2);
//        rDatabase.child(String.valueOf(str)).child("Location").child("startaddress").setValue(address);
//        rDatabase.child(String.valueOf(str)).child("Location").child("endaddress").setValue(address2);
//        rDatabase.child(String.valueOf(str)).child("Location").child("endaddress").setValue(address3);
//        rDatabase.child(String.valueOf(str)).child("Location").child("waitingcost").setValue(String.valueOf(mWaitingCostV));
//        rDatabase.child(String.valueOf(str)).child("Location").child("waitingtime").setValue(mTimer2.getText().toString().trim());
//        rDatabase.child(String.valueOf(str)).child("Location").child("triptime").setValue(mTimer1.getText().toString().trim());
//        rDatabase.child(String.valueOf(str)).child("Location").child("totaldistance").setValue(String.valueOf(distanceV));
//        rDatabase.child(String.valueOf(str)).child("Location").child("date").setValue(mTripDate.getText().toString());
//        rDatabase.child(String.valueOf(str)).child("Location").child("starttime").setValue(tripstarttime);
//        rDatabase.child(String.valueOf(str)).child("Location").child("tripcost").setValue(costTotalLast = Math.round(costTotalLast * 100.0) / 100.0);
//        rDatabase.child(String.valueOf(str)).child("Location").child("status").setValue(track_status);
...
//            SharedPreferences mPrefss = getSharedPreferences("IDvalue",0);
        ...
        SharedPreferences mPrefs5 = getSharedPreferences("IDvalue",0);
//        costTotalLast = Double.valueOf(mPrefs5.getString("tripcost","DEFAULT"));
...
//        SharedPreferences mPrefs = getSharedPreferences("IDvalue",0);
//        Double prevoiuswaitingCost = Double.valueOf(mPrefs.getString("waitingcost","0"));


        if ((distanceV * 1000) > vehicle_initial_distance*1000) {

//            waitingCost = Double.valueOf((((updatedTime2 / 1000)) * 1/15));
            waitingCost = Double.valueOf((int) (((updatedTime2 / 1000) / 1/15) * 1));
//            costTotalLast = waitingCost + 150 + ((distanceV * 1000) - 2000) * 48 / 1000;
            costTotalLast = waitingCost + vehicle_initial_charge + ((distanceV * 1000) - vehicle_initial_distance*1000) * vehicle_charge / 1000;
//            mTripCostV = 150 + ((distanceV * 1000) - 2000) * 48 / 1000;
            mTripCostV = vehicle_initial_charge + ((distanceV * 1000) - vehicle_initial_distance*1000) * vehicle_charge / 1000;
...
//            waitingCost = Double.valueOf((((updatedTime2 / 1000)) * 1/15));
            waitingCost = Double.valueOf((int) (((updatedTime2 / 1000) /  1/15) * 1));
            costTotalLast = waitingCost + vehicle_initial_charge;
            mTripCostV = vehicle_initial_charge;
            mWaitingCostV = Math.round(waitingCost * 100.0) / 100.0;
//            previousCost = 0.0;
        }
...
//                    String status=datas.child("tripstatus").getValue().toString();
//                    if(status == "Running"){
...
//        ref2.child(String.valueOf(maxid)).child("driverid").setValue(str+"-Completed");
    ...
    private Runnable updateTimerThread = new Runnable() {
        ...
        public void run() {
            ...
            if (hrs == 48) {
                stopTimer = true;
                Toast.makeText(FailedMapActivity.this, "You guys are awesome",
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
        switch (menuItem.getItemId()) {
            case R.id.nav_logout:
                final AlertDialog.Builder alert2 = new AlertDialog.Builder(FailedMapActivity.this);
                alert2
                        .setTitle("NexRide")
                        ...
                        .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int whichButton) {
                                ...
                                Intent intent = new Intent(FailedMapActivity.this, DriverLoginActivity.class);
                                startActivity(intent);
                                finish();
                            }
                        })
            ...
            case R.id.nav_history:
                ...
                Intent intent = new Intent(FailedMapActivity.this, DriverHistory.class);
                startActivity(intent);
                break;
...
//                break;
            case R.id.nav_googlemap:
                ...
                if (launchIntent != null) {
                    ...
                    Toast.makeText(FailedMapActivity.this, "There is no package available in android", Toast.LENGTH_LONG).show();
                }
                break;
        }
